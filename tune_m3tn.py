...
from models.architecture import M3TN
...
def valid(model, valid_dataloader, device, metric):
    ...
    for step, (X, T, valid_label) in enumerate(tqdm(valid_dataloader)):
        ...
        #有所区别的地方
        _, _, _, _, _, u_tau = model.module.model.forward(feature_list)
...
def objective(single_trial):
    ...
    num_experts = trial.suggest_categorical('num_experts', [3, 4, 5, 6, 7, 8])
    ...
    # model
    #model = M3TN(input_dim=12, h_dim=rank, is_self=False, act_type="elu", rep_norm=False)
    model = M3TN(input_dim=12, h_dim=rank, output_dim=rank2,num_experts=num_experts,
                is_self=False, act_type="elu")
    ...
    result_early_stop = 0
    logger.info(f'M3TN: Rank {local_rank} Start Training')
    for epoch in range(num_epoch):
        ...
        if local_rank == 0:
            logger.info("Epoch loss: {}, Avg loss: {}".format(tr_loss, tr_loss / tr_steps)) #有.format前面就不用加f
            # valid
            model.eval()
...
if local_rank == 0:
    ...
    trials.to_csv(table_path + 'criteo/' + 'M3TN-Tune.csv')
    ...
    hyper_params_dict['criteo']['M3TN'] = best_params
